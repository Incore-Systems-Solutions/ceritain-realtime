...
export function OTPModalI18n({ email, onBack }: OTPModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.errorCode === 0 && response.result) {
        ...
        setError(response.message || t("errorWrongOtp"));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t("errorWrongOtp"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <p className="text-sm text-gray-600 leading-relaxed">{t("footer")}</p>
        </motion.div>
      </motion.div>
    </motion.div>
  );
